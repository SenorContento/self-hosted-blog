...
class sqlCommandsPGP {
  ...
  public function readData($keyid) {
    try {
      ...
      $sql = "SELECT * FROM GPGKeys WHERE keyid LIKE concat('%', :keyid, '%') LIMIT 20";
      $query = $conn->prepare($sql);
      $query->execute(['keyid' => $keyid]);
      $results = $query->fetchAll();

      $keys = [];
      foreach($results as $key) {
        //print("ID: " . $key['id']);
        array_push($keys,["id" => $key['id'], "keyid" => $key['keyid'], "fingerprint" => $key['fingerprint'], "dateadded" => $key['dateadded']]);
      }
